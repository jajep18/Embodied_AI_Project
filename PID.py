...
sColorR = ev3.ColorSensor('in2') # Right color sensor
sColorL = ev3.ColorSensor('in3') # Left color sensor
...
sColorVal = [100-sColorL.value(), 100-sColorR.value()]
...
while True:
    ...
    w_a = mu * sColorVal[0]/100 * (sColorVal[0] - sColorVal_prev[0])/100
    w_b = mu * sColorVal[1]/100 * (sColorVal[1] - sColorVal_prev[1])/100
    Y_R = w_a * sColorVal[0]
    Y_L = w_b * sColorVal[1]
    print(str(sColorVal[0]) + " " +str(sColorVal[1]) + " " +str(w_a) + " " + str(w_b) + " " + str(Y_R) + " " + str(Y_L))

    #mLeft.duty_cycle_sp = 10 + Y_L
    #mRight.duty_cycle_sp = 10 + Y_R
